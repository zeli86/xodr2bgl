...
bool CXodrRoadGraphImpl::init(const std::string& sFilePath, const bool auto_connect)
{
    _auto_connect_lane_sections = auto_connect;
    clear();
...
void CXodrRoadGraphImpl::add_new_edges(const cRoadLaneSectionRow& row, int32_t& vertex_counter, int32_t& edge_counter)
{
    ...
    constexpr auto nobits = 8 * sizeof(row._lane) - 1;
    boost::optional<RoadNetEdgeItr> pred_edge, succ_edge, neighbor_edge;
    boost::optional<RoadNetVertex> v0, v1;

    if (row.has_missing_pred_section())
    {
        const auto left_neighbor = row._lane - (1 | (row._lane >> nobits));
        neighbor_edge = find(row._road, left_neighbor, row._section);
        if (neighbor_edge.has_value())
        {
            const auto& edge = *(neighbor_edge.value());
            v0 = source(edge, m_RoadNet);
        }
        else
        {
            const auto right_neighbor = row._lane + (1 | (row._lane >> nobits));
            neighbor_edge = find(row._road, right_neighbor, row._section);
            if (neighbor_edge.has_value())
            {
                const auto& edge = *(neighbor_edge.value());
    ...
    if (row.has_missing_succ_section())
    {
        const auto left_neighbor = row._lane - (1 | (row._lane >> nobits));
        neighbor_edge = find(row._road, left_neighbor, row._section);
        if (neighbor_edge.has_value())
...
void CXodrRoadGraphImpl::dfs(const int32_t road, const int32_t lane, const int32_t section, const bool src)
{
    auto edge = find(road, lane, section);

    if (edge.has_value() == true)
    {
        RoadNetVertex start;
        if (src == true)
        {
            start = source(**edge, m_RoadNet);
        }
        else
        {
            start = target(**edge, m_RoadNet);
        }

        dfs_visitor myvis;
        auto indexmap = boost::get(boost::vertex_index, m_RoadNet);
        auto colormap = boost::make_vector_property_map<boost::default_color_type>(indexmap);
        boost::depth_first_visit(m_RoadNet, start, myvis, colormap);
    }
}
...
void CXodrRoadGraphImpl::set_log_level(const unsigned lvl)
{
    ...
    log::core::get()->set_filter(log::trivial::severity >= lvl);
}
